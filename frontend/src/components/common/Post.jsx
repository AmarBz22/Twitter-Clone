...
const Post = ({ post, onDelete }) => {
  ...
  useEffect(() => {
    if (authUser && post?.likes.includes(authUser._id)) {
      setIsLiked(true);
    }
  }, [authUser, post?.likes]);
  ...
  const handleDeletePost = async () => {
    try {
      ...
      await axios.delete(`http://localhost:8000/api/post/${post._id}`, config);
      setIsDeleting(false);
      onDelete(post._id); 
  ...
  const isMyPost = authUser && postOwner && postOwner._id === authUser._id;
